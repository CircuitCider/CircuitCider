...
use crate::{shaders::neon_glow::NeonGlowMaterial, ui::{get_first_hit_with, BuildToolMode}};
...
pub fn spawn_gizmo_when_needed (
    ...
    if models_marked_for_gizmo.iter().len() > 0 && spawned_gizmos.iter().len() == 0 && *tool_mode == TRANSFORM_GIZMO_ACTIVE {
        let cube_size = 0.3;
